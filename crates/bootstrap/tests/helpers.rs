...
pub async fn get_account(client: &RpcClient, key: &Pubkey) -> anyhow::Result<Account> {
    client.get_account_with_commitment(key, CommitmentConfig::processed()).await.map_err(|e| anyhow::anyhow!("{:?}", e))?.value.ok_or(anyhow::anyhow!("Account not found: {:?}", key))
}
...
pub async fn get_token_account(
    ...
    let account = client.get_account_with_commitment(token_account, CommitmentConfig::processed()).await.map_err(|e| anyhow::anyhow!("{:?}", e))?.value.ok_or(anyhow::anyhow!(
            "Token account not found: {:?}",
            token_account
        ))?;
    TokenAccount::unpack(&account.data).map_err(|err| anyhow::anyhow!("Failed to unpack token account: {:?}", err))
}

pub async fn get_token_balance(client: &RpcClient, token_account: &Pubkey) -> u64 {
    get_token_account(client, token_account).await.unwrap().amount
}

pub async fn get_lamports(client: &RpcClient, key: &Pubkey) -> anyhow::Result<u64> {
    Ok(client.get_balance_with_commitment(key, CommitmentConfig::processed()).await.map_err(|e| anyhow::anyhow!("{:?}", e))?.value)
}
...
pub async fn sim_tx(
    ...
    let blockhash = client.get_latest_blockhash_with_commitment(CommitmentConfig::processed()).await?.0;
...
pub async fn send_tx(
    ...
    let blockhash = client.get_latest_blockhash_with_commitment(CommitmentConfig::processed()).await?.0;
    tx.sign(&signers.to_vec(), blockhash);
    let sig = match client.send_transaction_with_config(
        &tx,
...
pub async fn send_and_confirm_tx(
    ...
    client.confirm_transaction_with_spinner(&sig, &blockhash, CommitmentConfig::processed()).await?;
    Ok(sig)
}
