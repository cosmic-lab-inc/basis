...
describe('driftVaults', () => {
	...
	before(async () => {
		try {
			...
			const basisMintIxs = await createMintIxs(
				connection,
				payer.publicKey,
				usdcMint,
				6,
			...
			await sendAndConfirm(connection, payer, basisMintIxs, [basisMint]);
			console.log('created basis mint');
	...
	// 	const hoardSize = 2;
	// 	console.log(
	// 		`generating hoard of ${hoardSize} investors for Drift vault: ${protocolVaultName}`
	// 	);
