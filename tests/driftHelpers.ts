...
export async function mockUserUSDCAssociatedTokenAccount(
	...
	const userAtaExists = await provider.connection.getAccountInfo(
		userUSDCAccount
	);
	...
	if (res.isErr()) {
		throw new Error(
			`Error creating user ATA: ${JSON.stringify(
				res.error as TransactionError
			)}`
		);
	}
...
export async function createUsdcAssociatedTokenAccount(
	...
	if (res.isErr()) {
		throw new Error(
			`Error creating USDC ATA: ${JSON.stringify(
				res.error as TransactionError
			)}`
		);
	}
...
export const setFeedPrice = async (
	...
	try {
		...
		const confirm = await oracleProgram.provider.connection.confirmTransaction(
			strategy
		);
...
export async function bootstrapDevnetInvestor(params: {
	...
	if (!userAcct && deposit) {
		...
		const [sig, _] = await driftClient.initializeUserAccount(
			activeSubAccountId
		);
